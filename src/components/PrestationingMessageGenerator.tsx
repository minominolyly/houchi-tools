import { HOUCHI_GUNYU_MAPS, HOUCHI_KEIKOKU_MAPS } from "../constants/DataList";
...
export default function PrestationingMessageGenerator(props) {
  const [state, setState] =
    useState<PrestationingMessageGeneratorState>(initialState);
  const commands: string[] = [];
  commands.push(`【事前駐屯】${format(new Date(), "yyyy-MM-dd")}`);
  ...
  const groupedHouchiGunyuCastles: {
    label: string;
    options: HouchiCastle[];
  }[] = [];
  state.houchiGunyuMap.houchiCastles.forEach((houchiCastle: HouchiCastle) => {
    const group = groupedHouchiGunyuCastles.find(
      (g) => g.label === houchiCastle.level.toString()
    );
    if (group) {
      group.options.push(houchiCastle);
...
interface PrestationingMessageGeneratorState {
  note: string;
  dateString: string;
  houchiKeikokuMap: HouchiMap;
...
interface PrestationingMessageGeneratorProps {}
